    ...
    class ProductosDb
    {
        ...
        public void insertProdAdm(String json, frmMain frMain)
        {
            ...
            try
            {
                ...
                {
                    string pluAdm = getPluProd(pro.CodInt);
                    // TODO insert en adm si el rpoducto es nuevo y update si esta modificado
                    string descripcion_corta = string.Empty;
                    if (pluAdm != pro.CodInt)
                    {
                        OdbcConnection conexion = con.getConnect();
                        OdbcCommand insert = new OdbcCommand();
                        insert.Connection = conexion;
                        if (pro.Nombre.Length > 24)
                        {
                            descripcion_corta = pro.Nombre.Substring(0, 24);
                        }
                        else
                        {
                            descripcion_corta = pro.Nombre;
                        }
                        ...
                        insert.CommandText = "insert into productos "
                                    ...
                                    + "'" + fecha + "',"                        // fecha_ultmov
                                    + " 2,"                                 // estado
                                    + "'" + pro.Nombre.ToUpper() + "',"               // descripcion
                                    + "'" + descripcion_corta.ToUpper() + "',"               // descripcion_corta
                                    + "'" + pro.Nombre.ToUpper() + "',"               // descripcion_larga
                    ...
                    else
                    {
                        ...
                        update.CommandText = 
                         "update productos "
                        +"set estado=3, "
                        + "descripcion='" + pro.Nombre.ToUpper() + "', "
                        + "descripcion_corta='" + descripcion_corta.ToUpper() + "', "
                        + "descripcion_larga='" + pro.Nombre.ToUpper() + "', "
                        +"rubro="+getRubro(pro.Grupo)+", "
                        +"familia="+getFamilia(getRubro(pro.Grupo).ToString(), pro.Grupo) +", "
                        +"subfamilia=1, "
                        +"unidad_lista=1, "
                        + "simbolo_un_lista='" + pro.SUventa + "', "
                        +"moneda_venta=1, "
                        +"unidad_compra=1, "
                        + "simbolo_un_compra='" + pro.SUcompra + "', "
                        +"moneda_compra=1, "
                        +"cod_iva=1, "
                        +"cod_impto_adic1=0, "
                        +"cod_impto_adic2=0, "
                        ...
                        +"piezas_caja=0, "
                        +"unidad_fleje='UNIDAD', "
                        + "costo_compra=" + pro.CostoUC + ", "
                        + "ultimo_costo=" + pro.CostoUC + ", "
                        +"costo_adicional=0, "
                        +"flete=0, "
                        +"tipo_fletevta=0, "
                        +"fletevta=0, "
                        ...
                        +"cod_empresa=1 and cod_art= "+ pro.CodInt +""
                        ;

                        OdbcDataReader updaters = update.ExecuteReader();
                        frMain.listBoxLog.Items.Insert(0, "GRUPO: " + pro.Grupo + " - PLU: " + pro.CodInt + " - NOMBRE: " + pro.Nombre + " - ESTADO: Producto Actualizado");
                        pro.estadoResponce = "Producto Actualizado";
                        conexion.Close();
                    }
        ...
        public void insertStock(String json, frmMain frMain)
        {
                        ...
                        try
            {
                ...
                foreach (var pro in producto.Items)
                {
                    if (getPluInv(pro.CodInt) != pro.CodInt)
                    {
                        OdbcConnection conexion = con.getConnect();
                        OdbcCommand insert = new OdbcCommand();
                        insert.Connection = conexion;
        ...
        public void insertBodega(String codArt)
        {
            OdbcConnection conexion = con.getConnect();
            OdbcCommand insert = new OdbcCommand();
            insert.Connection = conexion;
            insert.CommandText = "insert into bodegastock (cod_empresa, cod_sucursal, cod_bodega, cod_art, stock, critico)" +
                                 " select bodegas.cod_empresa, bodegas.cod_sucursal, bodegas.cod_bodega, "+codArt+", 0, 0" +
                                 " from bodegas where cod_empresa = 1";
            OdbcDataReader inserts = insert.ExecuteReader();
        }
        ...
        public void insertSqlite(Producto producto)
        {
            try
            {
                ...
                string sql = "insert into resp_productos "
                            ...
                            +  prod.OnHand + "','"
                            + prod.estadoResponce + "','"
                            + thisDay.ToString("yyyy-MM-dd HH:mm:ss") + "');";
        ...
        public String getPluProd(String plu)
        {
        ...
        public String getPluInv(String plu)
        {
            String plu1 = String.Empty;
            OdbcConnection conexion = con.getConnect();
            OdbcCommand select = new OdbcCommand();
            select.Connection = conexion;
            select.CommandText = "select * from bodegastock "
                + " where cod_art = " + plu + ";";

     ...
     String strConn = @"Data Source=C:/admtosap/DataB.sqlite;Pooling=true;FailIfMissing=false;Version=3";
        ...
        public void addRespuesta(Respuesta respuesta)
        {
            try
            {
                ...
                String fechaDoc = respuesta.fechadoc.ToString("yyyy-MM-dd HH:mm:ss");

                SQLiteConnection myConn = new SQLiteConnection(strConn);
                myConn.Open();
                RespuestasDb respdb = new RespuestasDb();
                MensajeRespuesta mensaje = respdb.extraeMensaje(respuesta.xml.Replace("'", ""));
                if (respuesta.xml == "todavía no fue enviado a SAP, favor verificar los errores de envío")
                {
                    mensaje.errorMsg = "todavía no fue enviado a SAP, favor verificar los errores de envío";
                }               
                string sql = "insert into respuestas (fecha, tipodte, folio, mensaje, tiporesp, xml, json,mensajerror, fechadoc) " 
                            +"values ('" 
                            + fecha + "' , "
                            + respuesta.tipodete + "," 
                            + respuesta.folio + ",'" 

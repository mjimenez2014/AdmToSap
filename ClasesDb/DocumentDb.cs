...
namespace AdmToSap
{
    class DocumentDb
    {
        ...
        public List<Document> getDocuments()
        {
            ...
            while (reader.Read())
            {
                if (reader.GetInt32(reader.GetOrdinal("COD_BODEGA_DESTINO")) == 0 && reader.GetByte(reader.GetOrdinal("TIPO_CARGO")) == 52)
                {
           ...
           // + "and tipo_cargo > 0 and nro_cargo > 0"
        ...
        public List<Document> getDocumentsXFecha(DateTime inicial, DateTime final, DateTime horaIni, DateTime horafin)
        {
            OdbcConnection conexion = con.getConnect();
            List<Document> docs = new List<Document>();
            Empresa empresa = new Empresa();
            ...
            select.CommandText = "select * from cabezalventas "
            + "where cod_empresa= " + empresa.cod_empresa + " and cod_sucursal= " + empresa.cod_sucursal + " and tipo_cargo in (1,2,4,6,7,32,33,34,38,39,41,52,56) "
            + "and tipo_abono = 0 and anulada='N' and FECHA_EM between '"+fchInicial+" "+horaInicial+"' and '"+fchFinal+" "+horafinal+"' "
            + "order by tipo_cargo, nro_cargo;";
            OdbcDataReader reader = select.ExecuteReader();
            // cargo facturas
            while (reader.Read())
            ...
            + "where cod_empresa= " + empresa.cod_empresa + " and cod_sucursal_abono = " + empresa.cod_sucursal
                // + "and tipo_cargo > 0 and nro_cargo > 0"
            + " and tipo_abono in (22,61) and FECHA_EM between '" + fchInicial + " "+horaInicial+"' and '" + fchFinal + " "+horafinal+"' "
            + "and tipodefactura > 0 and anulada='N';";
            OdbcDataReader reader2 = select2.ExecuteReader();
            while (reader2.Read())
            {
        ...
        public Document cabezalDoc(OdbcDataReader reader)
        {
            ...
            Console.WriteLine(reader.GetDecimal(reader.GetOrdinal("NRO_CARGO")));
            if (reader.GetInt32(reader.GetOrdinal("TIPO_ABONO")) == 61)
            {
                doc.SalesPersonCode = convertAdmSap.getSalesPersonCode(reader.GetInt32(reader.GetOrdinal("COD_SUCURSAL_ABONO")));
            }
            else
            {
            ...
            else
            {
                ...
                if (reader.GetByte(reader.GetOrdinal("TIPO_CARGO")) != 7)
                {
                    doc.FolioNumber = reader.GetDecimal(reader.GetOrdinal("NRO_CARGO")).ToString();
                }

            }
            doc.DocDate = reader.GetDateTime(reader.GetOrdinal("FECHA_EM"));
            ...
            if (reader.GetInt32(reader.GetOrdinal("TIPO_ABONO")) == 61)
            {
                ...
                doc.udf.U_SEI_INREF = referencia.tipoDoc; 
                doc.udf.U_SEI_FOREF = referencia.folioDoc; 
                doc.udf.U_SEI_FEREF = referencia.fechaDoc;
                doc.udf.U_SEI_CREF = referencia.condicionDoc; //1,2,3
            }
            ...
            if (reader.GetInt32(reader.GetOrdinal("TIPO_ABONO")) == 61)
            {
                doc.items = getItems(doc.Cod_Empresa, doc.codSucursalAbono, doc.Caja, doc.tipoAbono, doc.numAbono ,doc.DocDate);
            }
            else
            {
               
                // Cargo el detalle
                doc.items = getItems(doc.Cod_Empresa, doc.Cod_Sucursal, doc.Caja, doc.Tipo_Cargo, reader.GetDecimal(reader.GetOrdinal("NRO_CARGO")),doc.DocDate);
        ...
        public List<Item> getItems(Byte codEmpresa, int codSucursal, int caja, int tipo, decimal numero, DateTime fecha )
        {
            ...
            select.CommandText = "select * from detalleventas"
                ...
                + " and CAJA = " + caja
                + " and TIPO = " + tipo
                + " and date_format(fecha, '%Y-%m-%d') = '" + fecha.ToString("yyyy-MM-dd") + "'"// TODO agregar fechas
                + " and NUMERO = " + numero +";";
        ...
        public Referencia getReferencia(int nroAbono)
        {
            ...
            select.Connection = conexion;
            select.CommandText = "select * from cabezalventas "
            + "where cod_empresa= " + empresa.cod_empresa + " and cod_sucursal_abono= " + empresa.cod_sucursal + " and tipo_abono = 61 "
            + "and anulada='N' "
            + "and nro_abono ="  + nroAbono + " "
            + "and tipo_cargo > 0 and nro_cargo >0 ";
            ...
            while (reader.Read())
            {
                ...
                referencia.folioDoc = reader.GetString(reader.GetOrdinal("NRO_ABONO")); 
                referencia.fechaDoc = reader.GetDateTime(reader.GetOrdinal("FECHA_EM")); 
                referencia.condicionDoc = reader.GetString(reader.GetOrdinal("cod_ref")); //TODO Marciano tiene que agregar columna

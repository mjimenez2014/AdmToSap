...
namespace AdmToSap
{
    class Procesos
    {
        ...
        {
            frmain.labelEstadoProceso.Text = "";
            frmain.labelEstadoProceso.Text = "Procesando.....";
            ...
            frmain.labelEstadoProceso.Text = "Proceso Terminado";
        }
        ...
        public void addDocuments(frmMain frmain)
        {
            ...
            foreach (Document p in documents)
            {
                ...
                String U_PAR_VALID = "        \"U_PAR_VALID\": \"" + p.SalesPersonCode + "-" + p.Indicator + "-" + p.FolioNumber + "-" + p.Caja + "\" ";

                if(p.tipoAbono == 61)
                {
                    folioNumber = "        \"FolioNumber\": \"" + p.numAbono + "\", ";
                    U_SEI_INREF = "        ,\"U_SEI_INREF\": \"" + convadmsap.typeDocument(Convert.ToInt32(p.udf.U_SEI_INREF)) + "\", ";
                    U_SEI_FOREF = "        \"U_SEI_FOREF\": \"" + p.udf.U_SEI_FOREF + "\", ";
                    U_SEI_FEREF = "        \"U_SEI_FEREF\": \"" +  String.Format("{0:yyyyMMdd}",p.udf.U_SEI_FEREF) + "\", ";
                    U_SEI_CREF = "        \"U_SEI_CREF\": \"" + p.udf.U_SEI_CREF + "\", ";
                    U_PAR_VALID = "        \"U_PAR_VALID\": \"" + p.SalesPersonCode + "-" + p.Indicator + "-" + p.FolioNumber + "-" + p.Caja + "\" ";
        ...
        public void addDocXFecha(frmMain frmain, DateTime inicial, DateTime final, DateTime horaIni, DateTime horaFin)
        {
            frmain.labelEstadoProceso.Text = "";
            frmain.labelEstadoProceso.Text = "Procesando.....";
            string cardcade = string.Empty;
            ...
            ConvertAdmSap convadmsap = new ConvertAdmSap();
            consqlite = condbsqlite.getConectSqlite();
            documents = docdb.getDocumentsXFecha(inicial,final, horaIni,horaFin);
            if (documents.Count == 0)
            {
                frmain.listBoxLog.Items.Insert(0, "No se encuentran Documentos");
            }
        ...
        public void addPayments(frmMain frmain)
        {
            ...
            foreach (Payment p in payments)
            {
               ...
               // if (p.CashAccount != "") p.CashSum = ""; // TODO
                string json = string.Empty;
                string cabecera = "{"
                              + "\"Payment\": {"
                              + "\"CardCode\": \"" + carCodeCFIB + "\","
            ...
            String  finjson  =     "]"
                   ...
                   // Console.WriteLine("CARDCODE: {0} DOCDATE: {1} ",p.CardCode +" | ",p.DocDate + " | ");
                    ...
                    if (p.documentsap.Count != 0)
                    {
                        Connect conn = new Connect();
                        responce = conn.HttpPOST(url, json);
                    }
                    else
                    {
                        responce = "Error";
                ...
                if (respdb.getMensajeError(responce) == -1)
                {
                    ...
                    if (responce == "Error")
                    {
                        respuesta.xml = "todavía no fue enviado a SAP, favor verificar los errores de envío";
                    }
                    else
                    {
                        respuesta.xml = responce.Replace("'", "");
                    ...
                    else
                    {
                        ...
                        if (responce == "Error")
                        {
                            respuestaSap = "Documento NO envíado a SAP el pago queda pendiente";
                        }
                        else
                        {
                            respuestaSap = respdb.extraeMensajeCliente(responce).ToString();
                        }
                        String evento = "ENVIO: Pagos - TIPO DOC: " + respuesta.tipodete + " - FOLIO PAGO: " + respuesta.folio + " - ESTADO: " + respuestaSap + " ERROR";
        ...
        public void addPaymentsXFecha(frmMain frmain, DateTime inicial, DateTime final, DateTime horaIni, DateTime horaFin)
        {
            PaymentDb paymentdb = new PaymentDb();
            List<Payment> payments = new List<Payment>();
            DocumentDb docdb = new DocumentDb();
            Log log = new Log();
            consqlite = condbsqlite.getConectSqlite();
            payments = paymentdb.getComprobantePagoXFecha(inicial, final,horaIni,horaFin);
            if (payments.Count == 0)
            {
                frmain.listBoxLog.Items.Insert(0, "No se encuentran mas Pagos");
            }
        ...
        public void getProductos(string fecha, string intervalo, frmMain frmain)
        {
            ...
            while (jsonResponce !=  j)
            {
               ...
               // MessageBox.Show(responce);
                ...
                if (jsonResponce == "{\"rowCount\":\"0\",\"Items\":[]}")
                {
        ...
        public void getProductosManual(string fecha, string first,string last, frmMain frmain)
        {
            frmain.labelEstadoProceso.Text = "";
            frmain.labelEstadoProceso.Text = "Procesando.....";
            //TODO checkbox es nulo fecha es igual a ""
        ...
        public void getInventarios(String fecha, string intervalo, frmMain frmain)
        {
            frmain.labelEstadoProceso.Text = "";
            frmain.labelEstadoProceso.Text = "Procesando.....";
            InventariosDb invDb = new InventariosDb();
            String bodega = invDb.getBodegaSap();
            int first = 0;
            int last = 0;
            consqlite = condbsqlite.getConectSqlite();
        ...
        public void getInventariosManual(String fecha, string first, string last, frmMain frmain)
        {
            frmain.labelEstadoProceso.Text = "";
            frmain.labelEstadoProceso.Text = "Procesando.....";
            InventariosDb invDb = new InventariosDb();
        ...
        public void getPrecios(string fecha,string intervalo, frmMain frmain)
        {
            ...
            while (jsonResponce != j)
            {
               ...
               // System.Console.WriteLine("LA RESPUESTA ES :" + responce);
                ...
                jsonResponce = respdb.extraeJsonPrecios(responce);
                if (jsonResponce != "vacio")
                {
                    preciosdb.upPrecAdm(jsonResponce);
                }
                lista += 1;
            }
            frmain.labelEstadoProceso.Text = "Proceso Terminado";
        ...
        public void getPreciosManual(string fecha, string first,string last, frmMain frmain)
        {
            frmain.labelEstadoProceso.Text = "";
            frmain.labelEstadoProceso.Text = "Procesando.....";
            int lista = 1;
            ...
            while (jsonResponce != j)
            {
             ...
             //   System.Console.WriteLine("LA RESPUESTA ES :" + responce);
                PreciosDb preciosdb = new PreciosDb();
                ListaPrecio precios = new ListaPrecio();
                jsonResponce = respdb.extraeJsonPrecios(responce);
                if (jsonResponce != "vacio")
                {
                   precios = preciosdb.upPrecAdm(jsonResponce);
                   foreach (var p in precios.Items)
                   {
                       frmain.listBoxLog.Items.Insert(0, "PLU: "+p.CodInt +" - PRECIO: " +p.Price + "");
                   }
                }
                else
                {
                    frmain.listBoxLog.Items.Insert(0, "No se encuentran Productos con precios modificados");
                }
                lista += 1;
            }
            frmain.labelEstadoProceso.Text = "Proceso Terminado";
        ...
        public void addJournalEntry(frmMain frmain)
        {
            ...
            foreach (JournalEntry jentry in jentries)
            {
                ...
                Connect conn = new Connect();
                String responce = "Error";//= conn.HttpPOST(url, json);
                //System.Console.WriteLine("LA RESPUESTA ES :" + responce);
        ...
        public void EnvioDiario(frmMain frmain)
        {
           ...
           // this.addDocXFecha(frmain, DateTime.Now, DateTime.Now,DateTime.Parse("2015-01-01 00:00:00"),DateTime.Parse("2015-01-01 23:59:59"));
            this.addPaymentsXFecha(frmain, DateTime.Now, DateTime.Now, DateTime.Parse("00:00:00"), DateTime.Parse("23:59:59"));
        }
